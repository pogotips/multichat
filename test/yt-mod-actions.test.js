...
import { makeHub, req } from './helpers/makeHub.js';
...
describe('handleIngestYt: {type:"mod"} dispatch', () => {
  ...
  // ROOMSTATE parity (PR #38's 2026-08-08 streak coverage audit, item 3): gray info
  // row, no kind, never buzzes/speaks (see tts.test.js emitCategory case).
  it('action:"mode_change" applies applyYtModeChange and renders a gray sys row', async () => {
    const { hub } = makeHub();
    const res = await hub.handleIngestYt(req({ type: 'mod', action: 'mode_change', text: 'Slow mode is on' }));
    expect(res.status).toBe(200);
    const row = hub.ring[hub.ring.length - 1];
    expect(row.sys).toBe('modechange');
    expect(row.text).toBe('Slow mode is on');
    expect(row.kind).toBeUndefined();
  });

  it('action:"mode_change" without text is rejected 400', async () => {
    const { hub } = makeHub();
    const res = await hub.handleIngestYt(req({ type: 'mod', action: 'mode_change' }));
    expect(res.status).toBe(400);
  });

  it('action:"mode_change" ellipsizes an oversized text', async () => {
    const { hub } = makeHub();
    const longText = 'x'.repeat(300);
    const res = await hub.handleIngestYt(req({ type: 'mod', action: 'mode_change', text: longText }));
    expect(res.status).toBe(200);
    const row = hub.ring[hub.ring.length - 1];
  ...
  it('the raid-queue caller is forbidden from mod actions (secret-scoping regression guard)', async () => {
    const { hub } = makeHub();
    hub.pushMessage('yt', { user: 'Alice', text: 'hi', ytId: 'yt-1' });
    const res = await hub.handleIngestYt(req({ type: 'mod', action: 'delete', ytId: 'yt-1' }, { caller: 'raidq' }));
  ...
  it('a missing/unknown caller header is forbidden from mod actions (fail closed)', async () => {
    const { hub } = makeHub();
    const res = await hub.handleIngestYt(req({ type: 'mod', action: 'delete', ytId: 'yt-1' }, { caller: null }));
    expect(res.status).toBe(403);
  });
  ...
  it('the raid-queue caller is unaffected for its normal (non-mod) chat POSTs', async () => {
    const { hub } = makeHub();
    const res = await hub.handleIngestYt(req({ user: 'RenewalBot', text: 'membership renewed' }, { caller: 'raidq' }));
    expect(res.status).toBe(200);
